sap.ui.define(
	...
	function (Message, MessageType, ValueState) {
		 ...
		 */
		var Validator = function (oi18nModel) {
			this._isValid = true;
		...
		/**
		 * Get localized text from i18n Model
		 * @memberof nl.qualiture.plunk.demo.utils.Validator
		 *
		 * @param {string} sMessage - i18n property name to get localized text
		 */
		Validator.prototype._getLocalizedText = function (sMessage) {
			if (this._oi18nModel) {
				var localizedText;

				try {
					localizedText = this._oi18nModel.getResourceBundle().getText(this._oi18nMessagesMap[sMessage]);
				} catch (ex) {
					localizedText = this._oi18nMessagesDefault[sMessage];
				}
				
				return localizedText;
			} else {
				return this._oi18nMessagesDefault[sMessage];
			}
		};
		...
		Validator.prototype._validate = function (oControl) {
			...
			if (
				...
				isValid = false;
				this._setValueState(oControl, ValueState.Error, this._getLocalizedText(this._ai18nMessageList.wrong_input));
			} else {
				...
				try {
					...
					if (!oExternalValue || oExternalValue === "") {
						this._setValueState(
							oControl,
							ValueState.Error,
							this._getLocalizedText(this._ai18nMessageList.please_fill_this_mandatory_field)
						);
						isValid = false;
					} else if (
						...
						this._setValueState(
							oControl,
							ValueState.Error,
							this._getLocalizedText(this._ai18nMessageList.please_choose_an_entry)
						);
						isValid = false;
					} else {
						oControl.setValueState(ValueState.None);
		...
		Validator.prototype._addMessage = function (oControl, sMessage) {
			...
			if (sMessage === undefined) {
				sMessage = this._getLocalizedText(this._ai18nMessageList.wrong_input);
			} // Default message
